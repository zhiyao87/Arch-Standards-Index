...
def build_readme(rows):
    total = len(rows)
    mand = [r for r in rows if is_mandatory(r)]
    rec = [r for r in rows if not is_mandatory(r)]
    gb = [r for r in rec if r["编号"].startswith("GB")]
    jg = [r for r in rec if r["编号"].startswith("JGJ")]
    ...
    w("| 标准类型 | 版权状态 | 法律依据 |")
    w("|---|---|---|")
    w("| 强制性国家标准（GB，非采标） | 不受著作权法保护 | 《著作权法》第 5 条：具立法、行政、司法性质的文件不适用本法 |")
    w("| 推荐性国家标准（GB/T） | **受著作权法保护** | 1999 年最高人民法院答复北京高院；国家版权局版权管理司意见 |")
    w("| 行业标准 / 地方标准（JGJ、DBJ 等） | **受著作权法保护** | 同上，均为推荐性标准 |")
    w("| 标准图集 | **受著作权法保护** | 归中国建筑标准设计研究院等编制单位 |")
    w("")
    ...
    w("|---|---|")
    w("| 收录标准总数 | %d |" % total)
    w("| 强制性工程建设规范 | %d |" % len(mand))
    w("| 推荐性国家标准（GB / GB/T） | %d |" % len(gb))
    w("| 推荐性行业标准（JGJ） | %d |" % len(jg))
    w("")
    w("数据来源：住房和城乡建设部官网公告、国家标准全文公开系统、全国标准信息公共服务平台，")
    ...
    w("| 官方链接 | 住建部官网发布页 | 链接可能因官网改版失效，失效时用编号在 mohurd.gov.cn 站内检索 |")
    w("")
...
def build_obsidian_index(rows):
    ...
    w("| 编号 | 名称 | 分类 | 实施日期 |")
    w("|---|---|---|---|")
    for r in sorted([x for x in rows if is_mandatory(x)], key=lambda x: x["编号"]):
    ...
    w("| 编号 | 名称 | 分类 | 实施日期 |")
    w("|---|---|---|---|")
    for r in sorted([x for x in rows if not is_mandatory(x) and x["编号"].startswith("GB")],
                    key=lambda x: x["编号"]):
        w("| [[%s]] | %s | %s | %s |" % (safe_filename(r), r["名称"], r["分类"], cell(r["实施日期"])))
    w("")
    w("## 行业标准")
